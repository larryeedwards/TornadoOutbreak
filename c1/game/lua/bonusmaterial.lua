...
function getidfromreqs(level, gametype, medal)

  logline("starting")
  if bonusmaterialreqs[level] ~= nil then
	if bonusmaterialreqs[level][gametype] ~= nil then	
	  if bonusmaterialreqs[level][gametype][medal] ~= nil then  
        return bonusmaterialreqs[level][gametype][medal]
      else
		return -1
      end
    else
...
function getname(bonusid)

  if bonusid ~= -1 then
    return bonusmaterial[bonusid].name
  else
    return "error"
  end
...
function getfilename(bonusid)

  if bonusid ~= -1 then
    return bonusmaterial[bonusid].filename
  else
    return "error"
  end
...
function getbonustype(bonusid)
  ...
  if bonusid ~= -1 then
    return bonusmaterial[bonusid].type
  else
    return -1
  end
