function on_postload()
    childbyoid(3469).externalgroundmatrix = this
end
...
function on_timer(name,count)
  if name=='ringshrink' then
    ...
    childbyoid(3469).volumepercentadjustment= radiusadj
  end
end
...
function on_initiategrab()
  ...
  fujita = this.fujitarating
   ...
   childbyoid(3469):reset()
   childbyoid(3469):start()
   steps = grabdurationmax/.1
   settimer('ringshrink',.1,steps)
   childbyoid(3469).volumepercentadjustment= ringstart
   childbyoid(3469).particlesizepercentadjustment= partsizestart
   childbyoid(3469).emissionpercentadjustment= emmisionstart
...
function on_cleanupspritegrab()
   childbyoid(3469):hardstop()
end

function on_cashgrabbedsprites(numsprites)

   on_cleanupspritegrab();
   childbyoid(3468):reset()
   childbyoid(3468):start()
--                                                            
...
function startshieldfx()
  ...
  childbyoid(3554):reset()
  childbyoid(3554):start()

end

function stopshieldfx()

     childbyoid(3555):reset()
     childbyoid(3555):start()
...
function on_tornadorankchanged(tornado, oldrank, newrank)
   ...
   childbyoid(3551).volumepercentadjustment= dirtringstart
   childbyoid(3551).particlesizepercentadjustment= dirtpartsizestart
   childbyoid(3551).emissionpercentadjustment= dirtemmisionstart
