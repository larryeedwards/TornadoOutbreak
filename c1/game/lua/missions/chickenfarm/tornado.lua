function on_postload()
    childbyoid(3463).externalgroundmatrix = this
end
...
function on_timer(name,count)
  if name=='ringshrink' then
    ...
    childbyoid(3463).volumepercentadjustment= radiusadj
  end
end
...
function on_initiategrab()
  ...
  fujita = this.fujitarating
   ...
   childbyoid(3463):reset()
   childbyoid(3463):start()
   steps = grabdurationmax/.1
   settimer('ringshrink',.1,steps)
   childbyoid(3463).volumepercentadjustment= ringstart
   childbyoid(3463).particlesizepercentadjustment= partsizestart
   childbyoid(3463).emissionpercentadjustment= emmisionstart
...
function on_cleanupspritegrab()
   childbyoid(3463):hardstop()
end

function on_cashgrabbedsprites(numsprites)

   on_cleanupspritegrab();
   childbyoid(3462):reset()
   childbyoid(3462):start()
--                                                            
...
function startshieldfx()
  ...
  childbyoid(3548):reset()
  childbyoid(3548):start()

end

function stopshieldfx()

     childbyoid(3549):reset()
     childbyoid(3549):start()
...
function on_tornadorankchanged(tornado, oldrank, newrank)
   ...
   childbyoid(3545).volumepercentadjustment= dirtringstart
   childbyoid(3545).particlesizepercentadjustment= dirtpartsizestart
   childbyoid(3545).emissionpercentadjustment= dirtemmisionstart
