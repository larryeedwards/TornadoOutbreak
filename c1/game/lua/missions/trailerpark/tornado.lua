function on_postload()
    childbyoid(2834).externalgroundmatrix = this
end
...
function on_timer(name,count)
  if name=='ringshrink' then
    ...
    childbyoid(2834).volumepercentadjustment= radiusadj
  end
end
...
function on_initiategrab()
  ...
  fujita = this.fujitarating
   ...
   childbyoid(2834):reset()
   childbyoid(2834):start()
   steps = grabdurationmax/.1
   settimer('ringshrink',.1,steps)
   childbyoid(2834).volumepercentadjustment= ringstart
   childbyoid(2834).particlesizepercentadjustment= partsizestart
   childbyoid(2834).emissionpercentadjustment= emmisionstart
...
function on_cleanupspritegrab()
   childbyoid(2834):hardstop()
end

function on_cashgrabbedsprites(numsprites)

   on_cleanupspritegrab();
   childbyoid(2833):reset()
   childbyoid(2833):start()
--                                                            
...
function startshieldfx()
  ...
  childbyoid(2919):reset()
  childbyoid(2919):start()

end

function stopshieldfx()

     childbyoid(2920):reset()
     childbyoid(2920):start()
...
function on_tornadorankchanged(tornado, oldrank, newrank)
   ...
   childbyoid(2916).volumepercentadjustment= dirtringstart
   childbyoid(2916).particlesizepercentadjustment= dirtpartsizestart
   childbyoid(2916).emissionpercentadjustment= dirtemmisionstart
