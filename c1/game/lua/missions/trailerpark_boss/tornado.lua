function on_postload()
    childbyoid(1552).externalgroundmatrix = this
end
...
function on_timer(name,count)
  if name=='ringshrink' then
    ...
    childbyoid(1552).volumepercentadjustment= radiusadj
  end
end
...
function on_initiategrab()
  ...
  fujita = this.fujitarating
   ...
   childbyoid(1552):reset()
   childbyoid(1552):start()
   steps = grabdurationmax/.1
   settimer('ringshrink',.1,steps)
   childbyoid(1552).volumepercentadjustment= ringstart
   childbyoid(1552).particlesizepercentadjustment= partsizestart
   childbyoid(1552).emissionpercentadjustment= emmisionstart
...
function on_cleanupspritegrab()
   childbyoid(1552):hardstop()
end

function on_cashgrabbedsprites(numsprites)

   on_cleanupspritegrab();
   childbyoid(1551):reset()
   childbyoid(1551):start()
--                                                            
...
function startshieldfx()
  ...
  childbyoid(1637):reset()
  childbyoid(1637):start()

end

function stopshieldfx()

     childbyoid(1638):reset()
     childbyoid(1638):start()
...
function on_tornadorankchanged(tornado, oldrank, newrank)
   ...
   childbyoid(1634).volumepercentadjustment= dirtringstart
   childbyoid(1634).particlesizepercentadjustment= dirtpartsizestart
   childbyoid(1634).emissionpercentadjustment= dirtemmisionstart
